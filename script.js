...
// 初始化折疊狀態：前三類預設展開，其他收合
let openMap = {};
sections.forEach((s, index) => {
  openMap[s.key] = index < 3;
});
...
function renderSections() {
  ...
  sections.forEach(section => {
    ...
    const filteredOptions = section.options.filter(option => 
      !searchKeyword || option.toLowerCase().includes(searchKeyword.toLowerCase())
    );

    // 如果搜尋狀態下，該類別沒有任何符合選項，則隱藏該大類別
    if (searchKeyword && filteredOptions.length === 0) {
      return; 
    }
